...
Use `us-overnight-a-stock-mapping` with its neutral source files. This target does not require vendor metadata by default.
...
Permission metadata is preserved in `adapter.json` under `target_permission_contract` and `target_native_contract` for reviewer visibility.
