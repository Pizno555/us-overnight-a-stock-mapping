...
| --- | --- | --- | --- | --- |
| Skill IR | missing | schema 2.0.0; targets 1 | 2.0 schema, root export, and target-neutral contract evidence | Keep IR as the source before target packaging. |
| Target Compiler | missing | 1/1 targets pass | OpenAI, Claude, generic, Agent Skills compatible, and VS Code contracts generated from IR | Deepen target-native transforms when provider clients expose stronger runtime APIs. |
| Output Eval Lab | missing | 7 cases; delta 100.0; exec 0; blind 7 | with-skill/baseline, assertions, execution evidence, blind A/B, failure taxonomy | Add more real-file and adversarial holdout cases as usage grows. |
...
| Benchmark Reproducibility | missing | artifacts 25; missing 20; failures 0 | Public methodology, reproducible commands, required artifacts, and failure disclosure are machine-checkable | Keep the manifest current with every benchmark, package, and release evidence change. |
| Runtime Conformance | missing | 1/1 targets pass | Target package structure, metadata, relative paths, and degradation notes pass | Keep target conformance fixtures updated as platform contracts change. |
...
| Permission Metadata | missing | 1/1 target probes pass; metadata fallback 1; installer enforcement 0 | Packaged adapters expose explicit permission metadata, residual risks, and installer enforcement evidence when available | Preserve residual-risk notes until real native enforcement exists. |
...
| Registry Distribution | pass | zip entries 82; install failures 0; permission failures 0 | Package metadata, archive checksum, package verification, and install simulation pass | Regenerate registry after package verification so checksums stay aligned. |
| Review Studio | pass | decision review; warnings 5; score 86 | One page shows gates, evidence paths, blockers, warnings, actions, waivers, and annotations | Resolve human/external warning gates before claiming full release readiness. |
...
- `output-eval-lab` (missing): Add more real-file and adversarial holdout cases as usage grows.
...
- existing evidence: `reports/output_quality_scorecard.json`, `reports/output_blind_review_pack.json`
- missing evidence: `evals/output/cases.jsonl`, `scripts/run_output_eval.py`, `scripts/run_output_execution.py`, `reports/output_execution_runs.json`
...
- existing evidence: `reports/benchmark_reproducibility.json`, `reports/benchmark_reproducibility.md`
- missing evidence: `reports/benchmark_methodology.md`, `evals/failure-cases.md`, `tests/verify_benchmark_reproducibility.py`
